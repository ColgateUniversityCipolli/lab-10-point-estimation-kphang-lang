...
ggdat = tibble(
  ...
  mutate(
    PMF = daltbinom(x = x, size = n, prob = p, theta = theta)
  )
```
...
**Solution**
Theta, again, makes win distributions more likely to cluster around extremes (0 or 11) when it is smaller (0.5, in part b), and makes win distributions more likely in the center (5-6) when it is larger. My assessment is that when theta is larger, it is smaller it is putting more emphasis on game-to-game results and skill, meaning that if a player has won several games in a row already they are more likely to win the next game (the opposite is also true). When theta is larger, games are more independent and the results are less reliant on each other.
...
**Solution**
```{r}
E.altbinom.k = function(k, size, prob, theta){
  x.vals = 0:size
...
ll.altham = function(par, data, size, mle=FALSE){
  ...
  ll = sum(log(daltbinom(x = data, size = size, prob = p, theta = theta))) 
...
winners = dat.pdc.cleaned |> 
  select(Player = Winner, LegsWon = WinnerLegs) #Winner rows

losers = dat.pdc.cleaned |>
  select(Player = Loser, LegsWon = LoserLegs) #Loser rows

player.data = bind_rows(winners, losers) #combine them
...
The estimated values of p and theta vary across players, reflecting differences in both overall performance and match dynamics. The parameter p represents the probability of winning an individual leg, so players with higher values of p.hat tend to win a greater proportion of legs and are generally stronger performers in the standings. The parameter theta captures dependence between legs. Values of theta > 1 indicate clustering of outcomes, meaning that players are more likely to win or lose multiple consecutive legs. In contrast, values of theta < 1 suggest alternation between players, leading to more tightly contested matches. Comparing these results to the empirical distribution of match outcomes, the presence of many close matches (e.g., 6–5 results) is consistent with models that allow for dependence between legs. Players with higher theta values tend to exhibit streakier performance, while those with theta closer to 1 behave more like the independent Binomial model. Overall, these results provide evidence that the independence assumption of the standard Binomial model may be too restrictive, and that Altham’s model offers a more flexible framework for capturing the dynamics of match play.
