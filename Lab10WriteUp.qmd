...
**Solution**
```{r}
dat.pdc.cleaned = dat.pdc.cleaned |>
  pivot_longer(cols = c(Winner, Loser),
               names_to = "Result",
               values_to = "Player") |>
    mutate(W = ifelse(Result == "Winner", WinnerLegs, LoserLegs),
            L = ifelse(Result == "Winner", LoserLegs, WinnerLegs))
  
ll.alt = function(par, w, l) {
  p = par[1]
  theta = par[2]
  
  if (p <= 0 || p >= 1 || theta <= 0) return(Inf) #manipulate the range of parameters
  
  -sum(log(daltbbinom(w, w + l, p, theta)))
}

fit_altham_player = function(df) {
  w = df$WinnerLegs
  l = df$LoserLegs
  
  fit = optim(
    par = c(0.5, 1),   # starting values
    fn = ll.alt,
    w = w,
    l = l
  )
  
  tibble(p.hat = fit$par[1],
        theta.hat = fit$par[2])
}
results = dat.pdc.cleaned |>
  group_by(Player) |>
  summarize(
    p.hat = fit_altham_player(cur_data())$p.hat,
    theta.hat = fit_altham_player(cur_data())$theta.hat
  )
